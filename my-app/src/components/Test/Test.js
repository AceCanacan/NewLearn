...
export const startRecording = async (setIsRecording, setMediaRecorder, processRecording) => {
  setIsRecording(true);
...
export const finishRecording = (mediaRecorder, setIsRecording, setIsLoading) => {
  if (mediaRecorder) {
...
export const processRecording = async (audioBlob, compareQuestion, setComparisonResult, setIsLoading) => {
...
export const compareQuestion = async (userQuestion, flashcards, currentCardIndex, typingMode, typedAnswer, setCorrectAnswers, setCorrectlyAnsweredQuestions, setComparisonResult, setNewAnswerProvided, setWasCorrect, setLastCorrectAnswer, setIsLoading) => {
  ...
  try {
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      if (result === 'yes') {
        if (!setCorrectlyAnsweredQuestions.has(currentCardIndex)) {
...
const Test = () => {
  ...
  return (
    <div className="test-yourself">
      <h3>{deckName}</h3>
      <button className="top-right-button" onClick={() => navigate(`/Review/${deckName}`)}>
        Review Mode
      </button>
      {flashcards.length > 0 ? (
        finished ? (
            ...
            <div className="flashcard">
              ...
              <div className="flashcard-buttons">
                ...
                {!typingMode && !isRecording && !isLoading && comparisonResult !== 'Incorrect' && (
                  <button onClick={() => startRecording(setIsRecording, setMediaRecorder, processRecording)}>Start</button>
                )}
                {typingMode && (
                  <>
                    <input
                    ...
                    />
                    <button
                      onClick={() => compareQuestion(typedAnswer, flashcards, currentCardIndex, typingMode, typedAnswer, setCorrectAnswers, setCorrectlyAnsweredQuestions, setComparisonResult, setNewAnswerProvided, setWasCorrect, setLastCorrectAnswer, setIsLoading)}
                ...
                {comparisonResult === 'Incorrect' && !isRecording && !isLoading && (
                  <button onClick={() => startRecording(setIsRecording, setMediaRecorder, processRecording)}>Try Again</button>
                )}
