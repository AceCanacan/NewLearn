...
function Deck() {
  ...
  const [deckDescription] = useState("");
    ...
    const fetchDecks = async () => {
      if (user) {
        ...
        for (const deckDoc of decksSnapshot.docs) {
          ...
          // Add console log here to display each deck's title and description
          console.log(`Deck Title: ${deckDoc.id}, Description: ${description}`);
        }
  ...
  const saveDeck = async (deckName, deckDescription) => {
    if (!decks[deckName] && user) {
      ...
      try {
        ...
        console.log("Deck saved with name:", deckName, "and description:", deckDescription);
  ...
  <section className="deck-recent-decks">
    <ul className="deck-grid">
      {Object.entries(decks).map(([deckName, { numCards, description }]) => (
  ...
  <div className="deck-modal-overlay">
    <div className="deck-modal-content">
      ...
      <p>
        You have created {totalCardsCreated} out of {MAX_CARDS} maximum allowed
        cards. This action cannot be undone. Are you sure you want to continue?
      </p>
      <p className="deck-modal-description">
        Please enter a unique name and a brief description for your new deck.
        This will help you organize your flashcards better.
      </p>
      <input
        type="text"
...
/>
      ...
      <div className="deck-modal-actions">
        <button className="deck-confirm-button" onClick={handleConfirmNewDeck}>
          Create Deck
        </button>
        <button
